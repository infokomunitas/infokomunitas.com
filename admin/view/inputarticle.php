...
<div id="content-container">
	<form id="validate-enhanced" class="form parsley-form" method="post" action="<?=$basedomain?>article/articleinp" enctype="multipart/form-data">
			...
			<div class="col-md-9">
				<div class="portlet">
					...
					<div class="portlet-content">
						...
						<div class="col-md-6">
							<div class="form-group">
							<label for="select-input">Jenis Artikel</label>
									<select name="categoryid" class="form-control">
										<?php
										/*
										foreach($data['category'] as $val){
										?>
											<option value='<?=$val['id']?>' <?=(isset($data['item'][0]['categoryid']) ? ($data['item'][0]['categoryid']==$val['id'] ? 'selected' : '') : '')?>><?=$val['category']?></option>
										<?php
										}*/
						...
						<div class="col-sm-12">
							...
							<label for="textarea-input">Deskripsi</label>
									...
									<input type="hidden" name="authorid" value="<?=(isset($data['item'][0]['authorid']) ? $data['item'][0]['authorid'] : $_SESSION['admin']['id']);?>" />
									<input type="hidden" name="articleid_old" value="<?=(isset($data['item'][0]['articletype']) ? $data['item'][0]['articletype'] : '0');?>" />
									<!-- hidden -->
					...
					<div class="portlet-content">
						<div class="form-group">
							...
							<div id="postdate" class="input-group date" data-auto-close="true" data-date-format="dd-mm-yyyy" data-date-autoclose="true">
								<input class="form-control" type="text" name="postdate" value="<?=(isset($data['item'][0]['postdate']) ? $data['item'][0]['postdate'] : '');?>" data-required="true">
						...
						<div class="form-group">
						<label for="text-input">Expire Date</label>
							<div id="expiredate" class="input-group date" data-auto-close="true" data-date-format="dd-mm-yyyy" data-date-autoclose="true">
								<input class="form-control" type="text" name="expiredate" value="<?=(isset($data['item'][0]['expiredate']) ? $data['item'][0]['expiredate'] : '');?>">
								<span class="input-group-addon"><i class="fa fa-calendar"></i></span>
							</div>
						...
						<div class="form-group">
							<div class="checkbox">
								<label>
									<input type="checkbox" name="articletype" class="" <?=(isset($data['item'][0]['articletype']) ? ($data['item'][0]['articletype']==1 || $data['item'][0]['articletype']==11  ? 'checked': '') : '')?>>
									Put on Slideshow
								</label>
							</div>
						</div>
