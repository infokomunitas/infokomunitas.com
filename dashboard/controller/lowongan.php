...
class lowongan extends Controller {
	...
	public function index(){
		...
		if ($data){
			foreach ($data as $key => $val){
				...
				$data[$key]['expiredateChange'] = changeDate($val['expiredate']);
			}
		}
		
		// pr($data);
		$_SESSION['pages'] = 'news';
		return $this->loadView('viewlowongan',$data);
	...
	public function addlowongan(){
		...
		return $this->loadView('inputlowongan',$data);
	}
	...
	public function articleinp(){
		...
		if(isset($_POST)){
            ...
            echo "<script>alert('Data berhasil di simpan');window.location.href='".$CONFIG['admin']['base_url']."lowongan/index'</script>";
            }
	}
	...
	public function articledel(){
		...
		echo "<script>alert('Data berhasil di hapus');window.location.href='".$CONFIG['admin']['base_url']."lowongan/index'</script>";
